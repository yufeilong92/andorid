...
*/
    ...
    //    List<QuestionTagRelationSqliteVo> listTags;
    List<QuestionCaseVo> mainlist;

    public List<CaseCardVo> getLists() {
       /* if (listTags == null || listTags.isEmpty()) {
//            ArrayList<QuestionTagRelationSqliteVo>
            listTags = mTagRelationHelp.queryTagRalationWithTag(mTagId);
            number = listTags.size();
        } else {
            if (listTags.size() < number) {
                *//*      ArrayList<QuestionTagRelationSqliteVo>*//*
                listTags = mTagRelationHelp.queryTagRalationWithTag(mTagId);
                number = listTags.size();
            }
        }*/
//        List<QuestionCaseVo> mQuerylist = mQuestionSqliteHelp.queryQuestionVoWithTagCase(listTags);
        if (mainlist==null||mainlist.isEmpty()){
            List<QuestionTagRelationSqliteVo> listTags = mTagRelationHelp.queryTagRalationWithTag(mTagId);
            mainlist = mQuestionSqliteHelp.queryQuestionVoWithTagMainCase(listTags);
        ...
        for (int i = 0; i < size; i++) {
            QuestionCaseVo mainvo = mainlist.get(i);
            //父数据
            CaseCardVo cardVo = new CaseCardVo();
            cardVo.setCasevo(mainvo);
            cardVo.setMaintype(1);
            cardVo.setType(1);
            List<QuestionCaseVo> childs = mQuestionSqliteHelp.queryQuestionCaseVoChildOne(mainvo.getQuestion_id(), mainvo.getChapter_id(), Integer.valueOf(mCourseId));
...
//        List<CaseCardVo> mCardLists = new ArrayList<>();
//        mTextUtil.doDataListCaseEventTwo(mCardLists, mQuerylist);
        return mCardLists;
    }
